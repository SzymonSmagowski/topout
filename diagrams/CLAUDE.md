...
- `service-topology.drawio` — System context: Browser → Next.js (Vercel) → Convex Cloud → OpenAI, plus the Python sidecar as a separate box with the bearer-auth boundary marked. Calls out that the sidecar has no Convex client (data flows IN only).
- `weekly-report-graph.drawio` — The 3-node LangGraph topology (`load_payload` → `analyze_stats` → `synthesize_narrative`) with typed Pydantic state.
